...
let Map_ = {
    ...
    refreshLayer: async function (
        ...
        for (var i = L_._layersOrdered.length - 1; i >= 0; i--) {
            if (
                ['vector', 'vectortile'].includes(L_.layers.data[L_._layersOrdered[i]].type) &&
                String(L_.layers.data[L_._layersOrdered[i]].name) === String(layerObj.name)
            ) {
...
function makeVectorTileLayer(layerObj) {
    ...
    if (urlSplit[0].toLowerCase() === 'geodatasets' && urlSplit[1] != null) {
        ...
        $.ajax( {
            ...
            if (Array.isArray(L_.layersOrdered)) {
                L_.layersLoaded[
                    ...
                    "sliced": 
                        function(properties, zoom) {
                            ...
                            var smallIcon = new L.Icon({
                                iconSize: hide ? 0 : [iconsize, iconsize],
                                iconAnchor: [iconsize/2, iconsize/2],
                                // Green arrows (arrows-disp) to distinguish from purple velocity arrows
    ...
    var vectorTileOptions = {
        ...
        rendererFactory: ('Velocities' == layerObj.display_name) ? L.svg.tile : L.svg.tile,
